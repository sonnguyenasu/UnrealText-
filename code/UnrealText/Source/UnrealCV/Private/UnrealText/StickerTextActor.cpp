...
void AStickerTextActor::GetWorldLocation(std::string FileName){
	// world space location for upper-left corner
    float center_x = (UL_X + BR_X) / 2.0f;
    float center_y = (UL_Y + BR_Y) / 2.0f;
    UL_X_shrink = center_x + (UL_X - center_x) * SanityCheckRange;
    ...
    FileHandle << (int32)ULPosition[0]<<","<<(int32)ULPosition[1]<<","<<(int32)ULPosition[2]<<","
			                <<(int32)URPosition[0]<<","<<(int32)URPosition[1]<<","<<(int32)URPosition[2]<<","
							<<(int32)BRPosition[0]<<","<<(int32)BRPosition[1]<<","<<(int32)BRPosition[2]<<","
							<<(int32)BLPosition[0]<<","<<(int32)BLPosition[1]<<","<<(int32)BLPosition[2]<<std::endl;
	FileHandle.close();
	UE_LOG(LogTemp, Warning, TEXT("Finished writing positions"));
