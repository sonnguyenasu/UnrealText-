...
void FObjectHandler::RegisterCommands()
{
	...
	CommandDispatcher->BindCommand(TEXT("vget /object/stickertext/location [uint] [str]"), 
		FDispatcherDelegate::CreateRaw(this, &FObjectHandler::GetMeshLocation), 
		"get the xyz position of the mesh");
    CommandDispatcher->BindCommand(TEXT("vget /printhello"), 
		FDispatcherDelegate::CreateRaw(this, &FObjectHandler::PrintHello), 
		"print hello");
...
FExecStatus FObjectHandler::GetAdjusted(const TArray<FString>& Args)
{
	...
	{	
		...
		UE_LOG(LogTemp, Warning, TEXT("All done, ready to write adjusted box ... this is a test whether compiler works?"));
		for (int32 text_id=0;text_id<text_num;text_id++)
		{
			if (FoundActors.Num() <= text_id)
			{
...
FExecStatus FObjectHandler::GetMeshLocation(const TArray<FString>& Args)
{
	if(Args.Num() == 2)
	{
		AStickerTextActor* TextInstance = GetStickerTextActor_I_th(FCString::Atoi(*Args[0]));
		if (TextInstance == nullptr)
		{
			return FExecStatus::Error(FString::Printf(TEXT("GetMeshLocation: instance index out of range!")));
		}
		// actual code
		std::string FileName = std::string(TCHAR_TO_UTF8(*(Args[1])));
		
		TextInstance->GetWorldLocation(FileName);
		//FileHandle.close();
		return FExecStatus::OK();
	}
	return FExecStatus::Error(FString::Printf(TEXT("GetMeshLocation: Absent args!")));
}

FExecStatus FObjectHandler::PrintHello(const TArray<FString>& Args){
	return FExecStatus::OK(FString::Printf(TEXT("Hello")));
}