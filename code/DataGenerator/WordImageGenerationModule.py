...
font_paths = {
    ...
    "Vietnamese":"../../../resources/fonts&corpus/Vietnamese/fonts/"
}
...
corpus_paths = {
    ...
    "Vietnamese": "../../../resources/fonts&corpus/Vietnamese/corpus/corpus.txt"
}
...
class WordRenderer(object):
    def __init__(self, *args, **kwargs):
        self.FontSize = kwargs.get('FontSize', [8, 32])
    ...
    def RenderCurvedTextLine(self, alpha, MaximumWidth, MaximumHeight, SpacingFactor=0.0):
        try:
            texts = []
            text_joint = ''
            MaxiWidthExpanded = int(MaximumWidth * (1+np.sqrt(1+4*np.abs(alpha))/2))
            MaxiHeightExpanded = int(np.sqrt(MaximumHeight/np.abs(alpha))*(1+np.sqrt(1+4*np.abs(alpha))))
            MaxiWidthExpanded = min(MaxiWidthExpanded, MaxiHeightExpanded)
            while True:
                # add one text
                word = self._LoadWord()
                _, h = self.CurrentFont.getsize(word) 
                ...
                w, h = self.CurrentFont.getsize(text_joint)
                MaxiHeightExpanded = int(MaximumHeight / h)
                if w > MaxiWidthExpanded :
                    exceeded = min(max(int((w - MaxiWidthExpanded) / w * len(text_joint)), 1), len(texts[-1]))
                    text_joint = text_joint[:-exceeded].strip()
                    texts[-1] = texts[-1][:-exceeded]
            ...
            CharImage = Image.new('RGBA', (w, int(h*1.25)), (0, 0, 0, 0))
            draw = ImageDraw.Draw(CharImage)
            if self.is_Arabic:
                draw.text_alignment = 'right'
                draw.text_antialias = True
                draw.text_kerning = 0.0
            for i in range(len(text_joint)):
                draw.text((i*w/len(text_joint), alpha*(i-len(text_joint)/2)*(i-len(text_joint)/2)),
                          text_joint[i],
                          fill='white',
                          font=self.CurrentFont)
            CharImg = np.asarray(CharImage).copy()
            ...
            return self.RenderCurvedTextLine(alpha,MaximumWidth, MaximumHeight)
    ...
    def getFontSize(self, Width, Height):
        ...
        print(self.FontSize)
        FontSize = np.random.randint(self.FontSize[0],self.FontSize[1])#max(int(min(Width, Height / row_num, current_max)), self.FontSize[0])#
        return FontSize
    ...
    def RenderWordImage(self,
        ...
        while True:
            ...
            alpha = a[1]*np.random.randn() + sgn*a[0]
            if curved:
                CharImg, texts, text_joint, cbox, bbox = self.RenderCurvedTextLine(alpha,Width,Height,SpacingFactor=np.random.uniform(0, 0.4))
            else:
    ...
    def RenderWordImageA(self,
        ...
        bg_arr = LightMap[UL_y:BR_y,UL_x:BR_x,:]#cv2.bitwise_and(LightMap, mask)#
        ...
        bg_img_ = cv2.resize(bg_arr[:,:,:3],(text_mask.shape[1],text_mask.shape[0]))
...
if __name__ == '__main__':
    ...
    for i in (range(40)):
        ...
        img, texts, cbox, bbox = Renderer.RenderWordImageA(660, 1000, (30, 30, 30, 240))
        text_mask = img[:,:,3]
