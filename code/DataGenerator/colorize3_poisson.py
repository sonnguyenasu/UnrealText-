...
class Colorize(object):
    ...
    def color(self, bg_arr, text_arr, hs, place_order=None, pad=20):
        ...
        for i in place_order[::-1]:
            ...
            bg_arr_1 = 255*np.ones((bg_arr.shape[0],bg_arr.shape[1],4))
            bg_arr_1[:,:,:3] = bg_arr
            bg_arr_1[l[0]:l[0]+w,l[1]:l[1]+h,:] = rdr0*255#rendered[-1]
            #text_patch = np.concatenate((text_patch,text_patch,text_patch),axis=-1)
            #bg_arr_1 = bg_arr[text_patch]
            bg_arr_1/=255
            #plt.imshow(bg_arr_1)
            #plt.title("bg_arr_1")
            #plt.show()
...
def main():
    ...
    text_m = cv.imread("word-0.png",0)
    #text_m = cv.resize(text_m, (text_m.shape[1]*3, text_m.shape[0]*3))
    res = np.zeros((img.shape[0],img.shape[1]))
    res[res.shape[0]//2-text_m.shape[0]//2:res.shape[0]//2+text_m.shape[0]-text_m.shape[0]//2,res.shape[1]//2-text_m.shape[1]//2:res.shape[1]//2+text_m.shape[1]-text_m.shape[1]//2] += text_m 
    #text_m = cv.resize(text_m, (img.shape[1], img.shape[0]))
    #print(img.shape, text_m.shape)
    min_h = np.load("min_h.npy")
    #print(min_h)
    img = Color.color(img, [text_mask], min_h) 
